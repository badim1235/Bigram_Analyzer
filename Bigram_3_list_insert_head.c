...
void add_hash_table_head(Node **hash_table,char *word1,char *word2, int index) 
{
    nodePointer current=hash_table[index];    
    while(current!=NULL){
        if(my_strcmp(current->bigram[0],word1)==0 
            && my_strcmp(current->bigram[1],word2)==0){
                ++current->count;
                return;
            }
            current=current->next;
...
void make_hash_table(Node **hash_table,char **words)
{
    for (int i=0;words[i+1]!=NULL;i++){
        ...
        add_hash_table_head(hash_table,words[i],words[i+1],index);
    }
}
